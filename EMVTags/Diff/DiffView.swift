...
struct DiffView: View {
    ...
    internal init(
        ...
        _showOnlyDifferent = .init(initialValue: true)
        
        diffInput(onlyDifferent: showOnlyDifferent)
    }
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: commonPadding) {
                ...
                Group {
                    if initialTags.count == 2 {
                        tagList2(for: diffResults.map(\.normalized))
                    } else {
        ...
        .onChange(of: showOnlyDifferent, perform: diffInput)
    ...
    @ViewBuilder
    private func tagList2(for normalizedDiffs: [(TagWithDiff?, TagWithDiff?)]) -> some View {
        LazyVStack(spacing: commonPadding) {
            ForEach(Array(normalizedDiffs.enumerated()), id: \.0) { (offset, row) in
                switch row {
                case (let left?, let right?):
                    HStack(spacing: commonPadding) {
                        TagRowView(tag: left.0, byteDiffResults: left.1)
                        TagRowView(tag: right.0, byteDiffResults: right.1)
                    }
                case (let left?, _):
                    HStack(spacing: commonPadding) {
                        TagRowView(tag: left.0, byteDiffResults: left.1)
    ...
    private func diffInput(onlyDifferent: Bool) {
        ...
        if onlyDifferent {
            diffResults = diffResults.filter(\.isDifferent)
        }
    ...
    private static func split(diffResults: [TagDiffResult]) -> [(TagWithDiff?, TagWithDiff?)] {
        diffResults.map(\.normalized)
    }
